...
namespace TestEstante
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Estante.GuardarEstante(est1);//Guarda estante 1 en D:\\MiEstante.txt
            Estante.GuardarEstante(est2);//Guarda Estante 2 en D:\\MiEstante.txt
            Estante.Serializar(est1);//Guarda estante 1 en \Desktop\MiEstanteXML.xml NO FUNCIONA
            ...
            est1.GetProductos().Sort(Program.OrdenarProductos); //Metodo Sort
            Console.WriteLine(Estante.MostrarEstante(est1));
            est1 = est1 - Producto.ETipoProducto.Galletita; 
            Console.WriteLine("Estante1 sin Galletitas: {0}", Estante.MostrarEstante(est1)); Console.WriteLine("Contenido Estante2:\n{0}", Estante.MostrarEstante(est2));
        ...
        private static int OrdenarProductos(Producto proUno, Producto proDos)//compara 2 string para ordenar por el metodo Sort
        {
            return string.Compare(((int)proUno).ToString(),((int)proDos).ToString());
            //return String.Compare((int)proUno), ((int)proDos);
        }
    }
}
